  ...
  return new Promise((resolve, reject) => {
    ...
    reader.onload = (e) => {
      ...
      img.onload = () => {
        ...
        // Check if compressed size is still too large (>900KB to be safe)
...
// Helper function to convert image file to base64 (deprecated - use compressImage instead)
...
export const adminDB = {
  ...
  async getAllAreas() {
    try {
      const q = query(collection(db, COLLECTIONS.AREAS), orderBy('city', 'asc'));
      const snapshot = await getDocs(q);
      return snapshot.docs.map(doc => ({ id: doc.id, ...doc.data() }));
    } catch (error) {
      console.error('Error fetching areas:', error);
  ...
  async getAllTestimonials() {
    try {
      const q = query(collection(db, COLLECTIONS.TESTIMONIALS), orderBy('created_at', 'desc'));
      const snapshot = await getDocs(q);
      return snapshot.docs.map(doc => ({
        id: doc.id,
        ...doc.data(),
